...
const OnewaySearch = () => {
    ...
    const getDateForAmadeus = () => {
        const tripDate = searchflights[2].tripValue === 'Return'
            ? searchflights[4]?.rtndate?.startDate
            : searchflights[3]?.owdate
  ...
  return (
    <div className="flight-item-con">
        ...
        {isLoading ? (
            Array.from({ length: 5 }).map((_, index) => <FlightitemSkeleton key={`ow-skeleton-${index}`} />)
        ) : (
